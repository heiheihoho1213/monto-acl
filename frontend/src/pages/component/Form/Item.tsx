...
class FormItemComponent {
  static Input(props: { field: IFormItem }) {
    ...
    return (
      <Input style={{ width: '100%' }} placeholder={field.placeholder} disabled={field.disabled} {...field.option} />
    );
  }
  ...
  static TextArea(props: { field: IFormItem }) {
    ...
    return <Input.TextArea style={{ width: '100%' }} placeholder={field.label} {...field.option} />;
  }

  static NumberInput(props: { field: IFormItem }) {
    const { field } = props;

    return <InputNumber style={{ width: '100%' }} placeholder={field.label} {...field.option} />;
  }
  ...
  static Radio(props: { field: IFormItem }) {
    ...
    return <Radio.Group options={field.options} optionType='button' buttonStyle='solid' {...field.option} />;
  }

  static Switch(props: { field: IFormItem }) {
    const { field } = props;

    return <Switch {...field.option} />;
  }

  static Date(props: { field: IFormItem }) {
    const { field } = props;

    return <DatePicker {...field.option} />;
  }

  static DateRange(props: { field: IFormItem }) {
    const { field } = props;

    return <RangePicker {...field.option} />;
  }
